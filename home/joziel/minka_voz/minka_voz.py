...
def main():
    ...
    if session:
        linea(f"{G}  Sesión activa: {session['username']}{R}")
        master_password = sec._solicitar_master_password(session)
    else:
        # Pantalla de login
    ...
    # Configurar base de datos de usuario
    user_db_path = os.path.join(sec.HIDDEN_DB_DIR, f"user_{session['user_id']}.db")
    db.set_user_db(user_db_path, master_password)
    db.init_user_db()
    
    # Cargar configuración del usuario
    default_mode = db.get_user_setting("default_mode", "k2e")
    
    banner()
    linea(f"{G}  Bienvenido, {session['username']}{R}")
