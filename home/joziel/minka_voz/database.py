...
Soporte multi-usuario con encriptación
"""
...
# Base de datos de usuario (oculta)
_user_db_path = None
...
def set_user_db(db_path: str, master_password: str = None):
    """Configura la base de datos del usuario actual"""
    global _user_db_path, _master_password
    _user_db_path = db_path
    _master_password = master_password

def init_user_db():
    """Inicializa la base de datos del usuario"""
...
def add_favorite(word_id: int):
    """Agrega palabra a favoritos"""
    if not _user_db_path:
        return
    con = sqlite3.connect(_user_db_path)
    cur = con.cursor()
    cur.execute("""
        INSERT OR IGNORE INTO user_favorites (word_id, added_at) VALUES (?, ?)
    """, (word_id, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    con.commit()
    con.close()

def remove_favorite(word_id: int):
    """Elimina palabra de favoritos"""
    if not _user_db_path:
        return
    con = sqlite3.connect(_user_db_path)
    cur = con.cursor()
...
def get_favorites():
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT f.word_id, d.kogui, d.spanish, f.added_at
...
def save_user_note(word_id: int, note: str):
    """Guarda nota para una palabra"""
    if not _user_db_path:
        return
    con = sqlite3.connect(_user_db_path)
    cur = con.cursor()
    cur.execute("""
        INSERT INTO user_notes (word_id, note, created_at) VALUES (?, ?, ?)
